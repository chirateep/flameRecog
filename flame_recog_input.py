...
NUM_CLASSES = 10
NUM_PER_EPOCH_FOR_TRAIN = look ubuntu
NUM_PER_EPOCH_FOR_EVAL = look ubuntu

def read_flame_image(filename_queue):
	
	class FlameRecogRecord(object)
		pass
	result = FlameRecogRecord()

	label_bytes = 1 # 10 class if 100 class use 2
	result.height = 720
	result.width = 700
	result.depth = 3
...
def distorted_inputs(data_dir, batch_size):
	...
	read_input = read_flame_image(filename_queue)
...
def inputs(eval_data, data_dir, batch_size):
	...
	resized_image = tf.image.resized_image_with_crop_or_pad(
		reshape_image, width, height)

	float_image = tf.image.per_image_standardization(resized_image)

	float_image.set_shape([height, width, 3])
	reshape_image.label.set_shape([1])
